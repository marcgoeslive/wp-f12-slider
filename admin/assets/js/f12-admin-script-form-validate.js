function F12FormValidate(jQueryObject) {
    ...
    this.trigger_on_submit = function (e) {
        ...
        this.m_jQueryObject.find(".f12-form-validate").each(function () {
            ...
            if (typeof(validation) === "undefined" || validation.length <= 0) {
                if (c.is_textarea(jQuery(this)) == false) {
                    return;
                } else {
            ...
            if (c.validate(e, jQuery(this), JSON.parse(validation)) != true) {
                e.preventDefault();
            }
        });
    };
    ...
    this.check_condition = function (jQDOM, args) {
        ...
        for (var i = 0; i < args.length; i++) {
            ...
            switch(args[i].type){
                case 'checkbox':
                    element = jQuery(document).find("[name='" + args[i].name + "']:checked");
                ...
                case 'text':
                default:
                    element = jQuery(document).find("[name='" + args[i].name + "']");
                    //console.log(element.val()+":"+args[i].value);
                    if(element.val() != args[i].value){
                        ret = false;
                    }
    ...
    this.display_error_message = function (jQDOM, key, message) {
        ...
        if (jQuery(".f12-validation-error-message").length == 0) {
...
jQuery.fn.F12FormValidate = function () {
    new F12FormValidate(this);
};

jQuery(document).ready(function () {
    jQuery("form#post").F12FormValidate();
});